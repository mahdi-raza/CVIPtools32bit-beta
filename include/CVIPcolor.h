...
extern "C"{
   ...
   extern Image * colorxform(const Image *rgbImage, COLOR_FORMAT newcspace,
    ...
    extern Image * pseudo_remap(Image *image, byte **lookup_table);
   ...
   extern void rgb2yiq(Image *in, int inv);
	extern Image *color_contrast_enhance(Image* inputImage, int low_limit, int high_limit, float low_clip, float high_clip);
