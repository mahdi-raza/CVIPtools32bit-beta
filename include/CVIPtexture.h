...
/* [0] -> 0 degree, [1] -> 45 degree, [2] -> 90 degree, [3] -> 135 degree,
   [4] -> average, [5] -> range (max - min) */
	float energy[6];		/*  (1) Energy */
	float inertia[6];	   /*  (2) Inertia */
	float correlation[6];	/*  (3) Correlation */
	float IDM[6];		/*  (4) Inverse Diffenence Moment */
	float entropy[6];	/*  (5) Entropy */
	} TEXTURE2;
...
extern TEXTURE* texture(const Image* ImgP, const Image* segP, int band, int row, int col, long int hex_equiv, int distance);
extern TEXTURE2* texture2(Image *image, Image *labeled_image, int band, int row, int col, int distance,
                   int energy, int inertia, int correlation, int inv_diff, int entropy, int zerorowcol);
