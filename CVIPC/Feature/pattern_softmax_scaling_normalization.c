...
* =========================================================================
*
*             File Name: pattern_range_normalize.c 
*		  Expanded Name: Pattern classification range normalization
*			 Parameters: Pointers pointing to the file names of training and 
*						 test set, constant r, error code
*				 Return: error code
...
int *pattern_softmax_scaling_normalization(char *file_tr, char *file_tt, float r_softmax, int *err_code)
{	
	float	**vector;
	float	*b;
	float	*b1;
	...
	{
		printf("Error! pattern_softmax_scaling_normalization.c. Can't open , read or create file.");
		printf("%s\n",file_tr);
		*err_code = 7;
		return(err_code);
	}
	...
	if(i>6)
	{
		printf("Error! pattern_softmax_scaling_normalizaiion.c. There is more than one header in the training set.");
		printf("%s\n",file_tr);
		fclose(fp_tr);
		fclose(fp_tt);
		*err_code = 13;
	...
	if(i>6)
	{
		printf("Error! pattern_softmax_scaling_normalizaiion.c. There is more than one header in the test set.");
		printf("%s\n",file_tt);
		fclose(fp_tr);
		fclose(fp_tt);
		*err_code = 13;
	...
	while ((ch_tt != '*') || (ch_tr != '*'))
	{
		...
		{
			printf("Error! pattern_softmax_scaling_normalization.c. The header in the training and test files don't match.");
			fclose(fp_tr);
			fclose(fp_tt);
			*err_code = 1;
			return(err_code);
	...
	if(!vector)
	{
		printf("\nError! pattern_softmax_scaling_normalization.c. Memory allocation error.");
		fclose(fp_tr);
		fclose(fp_tt);
		*err_code = 9;
		return(err_code);
...
//********************training file******************************
	...
	for(m=0;m<j;m++)
	{
		...
		if((int)ch == 10)
		{
			printf("Error! pattern_softmax_scaling_normalization.c. Every entry in training set should have a class name.");
			fclose(fp_tr);
			fclose(fp_tt);
			fclose(fp_tr_temp);
			*err_code = 10;
	...
	// Softmax Scaling Normalization procedure called 
	vector = softmax_scaling(vector,i,j,r_softmax,0,err_code);
	if(*err_code !=0)
	{
		fclose(fp_tt);
		fclose(fp_tr_temp);
...
//********************test file************************************************
	...
	// Softmax Scaling Normalization procedure called 
	vector = softmax_scaling(vector,i,jj+2,r_softmax,1,err_code);
	if(*err_code !=0)
	{
		fclose(fp_tt_temp);
		for(n=0;n<size;n++)
