...
* =========================================================================
*
*             File Name: pattern_min_max_normalization.c 
*		  Expanded Name: Pattern classification min max normalization
*			 Parameters: Pointers pointing to the file names of training and 
*						 test set, specified min and max, error code
*				 Return: error code
...
extern float **min_max_normalization(float **vector, int i, int j, float s_min, float s_max, int option, int *err_code);

int *pattern_min_max_normalization(char *file_tr, char *file_tt, float s_min, float s_max, int *err_code)
{
	float	**vector;
	float	*b;
	float	*b1;
	...
	{
		printf("Error! pattern_min_max_normalization.c. Can't open , read or create file.");
		printf("%s\n",file_tr);
		*err_code = 7;
		return(err_code);
	}
	...
	if(i>6)
	{
		printf("Error! pattern_min_max_normalization.c. There is more than one header in the training set.");
		printf("%s\n",file_tr);
		fclose(fp_tr);
		fclose(fp_tt);
		*err_code = 13;
	...
	if(i>6)
	{
		printf("Error! pattern_min_max_normalization.c. There is more than one header in the test set.");
		printf("%s\n",file_tt);
		fclose(fp_tr);
		fclose(fp_tt);
		*err_code = 13;
	...
	while ((ch_tt != '*') || (ch_tr != '*'))
	{
		...
		{
			printf("Error! pattern_min_max_normalization.c. The header in the training and test files don't match.");
			fclose(fp_tr);
			fclose(fp_tt);
			*err_code = 1;
			return(err_code);
	...
	if(!vector)
	{
		printf("\nError! pattern_min_max_normalization.c. Memory allocation error.");
		fclose(fp_tr);
		fclose(fp_tt);
		*err_code = 9;
		return(err_code);
	...
	class_name = malloc((size-2)*sizeof(char *));
	if(!class_name)
	{
		printf("\nError! pattern_min_max_normalization.c. Memory allocation error.");
		fclose(fp_tr);
...
//********************training file******************************
	...
	for(m=0;m<j;m++)
	{
		...
		if((int)ch == 10)
		{
			printf("Error! pattern_min_max_normalization.c. Every entry in training set should have a class name.");
			fclose(fp_tr);
			fclose(fp_tt);
			fclose(fp_tr_temp);
			*err_code = 10;
	...
	// Min-Max Normalization procedure called 
	vector = min_max_normalization(vector,i,j,s_min,s_max,0,err_code);
	if(*err_code !=0)
	{
		fclose(fp_tt);
		fclose(fp_tr_temp);
...
//********************test file************************************************
	...
	// Pass the normalization information to vector[][]
	for(n=0;n<i;n++)
	{
		vector[jj][n] = b[n];
	}
	...
	// // Min-Max Normalization procedure called 
	vector = min_max_normalization(vector,i,jj+2,s_min,s_max,1,err_code);
	if(*err_code !=0)
	{
		fclose(fp_tt_temp);
		for(n=0;n<size;n++)
