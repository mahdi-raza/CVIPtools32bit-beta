...
* =========================================================================
*
*             File Name: pattern_city_block.c 
*		  Expanded Name: Pattern classification city block distance
...
int pattern_city_block(char *file_tt, char *file_tr, int *err_code)
{
	float	*a,*b;
	float	city_block_value = 0.0;
	int		r, c, temp; 
	int		i,j,jj,m,n;
	int		g=0;
	char	ch_tr, ch_tt;
	...
	{
		printf("Error! pattern_city_block.c. Can't open , read or create file.");
		printf("%s\n",file_tr);
		*err_code = 7;
		return(1);
	}
	...
	if(i>6)
	{
		printf("Error! pattern_city_block.c. There is more than one header in the training set.");
		printf("%s\n",file_tr);
		fclose(fp_tr);
		fclose(fp_tt);
		fclose(fp_temp);
	...
	if(i>6)
	{
		printf("Error! pattern_city_block.c. There is more than one header in the test set.");
		printf("%s\n",file_tt);
		fclose(fp_tr);
		fclose(fp_tt);
		fclose(fp_temp);
	...
	while ((ch_tt != '*') || (ch_tr != '*'))
	{
		...
		{
			printf("Error! pattern_city_block.c. The header in the training and test files don't match.");
			fclose(fp_temp);
			fclose(fp_tr);
			fclose(fp_tt);
			*err_code = 1;
	...
	{	
		...
		// Gets a number
		fscanf(fp_tt, "%f", &city_block_value);
	...
	if(!a)
	{
		printf("\nError! pattern_city_block.c. Memory allocation error.");
		fclose(fp_temp);
		fclose(fp_tr);
		fclose(fp_tt);
		*err_code = 9;
	...
	for(m=0;m<jj;m++)
	{
        ...
        for(n=0;n<j;n++)
		{
			...
			if((int)ch_tr == 10)
			{
				printf("Error! pattern_city_block.c. Every entry in training set should have a class name.");
				fclose(fp_temp);
				fclose(fp_tr);
				fclose(fp_tt);
				*err_code = 10;
			...
			// Calls the city block distance measure function
			city_block_value = city_block(a, b, i);
			...
			fprintf(fp_temp,"%d %d %f ", r, c, city_block_value);
