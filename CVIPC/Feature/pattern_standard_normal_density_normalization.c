...
* =========================================================================
*
*             File Name: pattern_standard_normal_density_normalization.c 
*		  Expanded Name: Pattern classification standard normal density normalization
...
int *pattern_standard_normal_density_normalization(char *file_tr, char *file_tt, int *err_code)
{	
	float	**vector;
	float	*b;
	float	*b1;
	...
	{
		printf("Error! pattern_standard_normal_density_normalization.c. Can't open , read or create file.");
		printf("%s\n",file_tr);
		*err_code = 7;
		return(err_code);
	}
	...
	if(i>6)
	{
		printf("Error! pattern_standard_normal_denstiy.c. There is more than one header in the training set.");
		printf("%s\n",file_tr);
		fclose(fp_tr);
		fclose(fp_tt);
		*err_code = 13;
	...
	if(i>6)
	{
		printf("Error! pattern_standard_normal_denstiy.c. There is more than one header in the test set.");
		printf("%s\n",file_tt);
		fclose(fp_tr);
		fclose(fp_tt);
		*err_code = 13;
	...
	while ((ch_tt != '*') || (ch_tr != '*'))
	{
		...
		{
			printf("Error! pattern_standard_normal_density_normalization.c. The header in the training and test files don't match.");
			fclose(fp_tr);
			fclose(fp_tt);
			*err_code = 1;
			return(err_code);
	...
	if(!vector)
	{
		printf("\nError! pattern_standard_normal_density_normalization.c. Memory allocation error.");
		fclose(fp_tr);
		fclose(fp_tt);
		*err_code = 9;
		return(err_code);
...
//********************training file******************************
	...
	for(m=0;m<j;m++)
	{
		...
		if((int)ch == 10)
		{
			printf("Error! pattern_standard_normal_normalization.c. Every entry in training set should have a class name.");
			fclose(fp_tr);
			fclose(fp_tt);
			fclose(fp_tr_temp);
			*err_code = 10;
	...
	// Standard Normal Density Normalization procedure called 
	vector = standard_normal_density_normalization(vector,i,j,0,err_code);
	if(*err_code !=0)
	{
		fclose(fp_tt);
		fclose(fp_tr_temp);
...
//********************test file************************************************
	...
	// Standard Normal Density Normalization procedure called 
	vector = standard_normal_density_normalization(vector,i,jj+2,1,err_code);
	if(*err_code !=0)
	{
		fclose(fp_tt_temp);
		for(n=0;n<size;n++)
