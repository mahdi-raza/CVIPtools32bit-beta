...
* =========================================================================
*
*             File Name: nearest_neighbor.c 
*		  Expanded Name: Nearest neighbor function
...
*						 similarity between each sample in the test set and all the 
*						 samples in the training set, uses nearest neighbor
*						 algorithm to find the closest match sample in training set 
*						 to the sample in the test set, calculates the STATISTICS, 
...
int nearest_neighbor(char *file_tt, char *file_tr, char *file_out, int option_normalize, int option_distance, float s_min, float s_max, float r_softmax, int r_minkowski)
{	
	struct temp
	{		
		char	data1[MAX_CASE];
	...
	*err_code = 0;	
	...
	{
		printf("Error! nearest_neighbor.c. Can't open , read or create file.");
		printf("%s\n",file_tr);
		return(7);
	}	
	...
	fprintf(fp_out,"%s","Classification Algorithm: Nearest Neighbor\n");	

	// Calls normalization function to perform normalization
	...
	case 5:	
		...
		break;
	default:
		printf("Error! nearest_neighbor.c. Option selection error on normalization.");
		fclose(fp_out);
		return(6);
		break;
	}

	// Performs distance or similarity measure
	...
	{
		printf("Error! nearest_neighbor.c. Option selection error on distance or similarity measure.");
		printf("%s\n",file_temp);
		fclose(fp_out);
		return(7);
	}
	...
	if(!class_st_tt)
	{
		printf("\nError! nearest_neighbor.c. Memory allocation error.");
		fclose(fp_out);
		fclose(fp);
		return(9);
	}
	...
	// Nearest Neighbor algorithm starts for similarity measure 

	if((option_distance == 4) || (option_distance == 5))
	{
		// Reads one sample and compares it with the previous read sample. Keeps the closest sample in the test set
		// If the two samples are found to be equal, then the lastest(newer) sample is chosen 
		g = 0;
		...
		// STATISTICS calculation and printing the result
	...
	// Nearest Neighbor algorithm starts for distance measure
