...
* =========================================================================
*
*             File Name: pattern_euclidean.c 
*		  Expanded Name: Pattern classification euclidean distance
...
int pattern_euclidean(char *file_tt, char *file_tr, int *err_code)
{
	float	*a,*b;
	float	euclidean = 0.0;
	...
	if(i>6)
	{
		printf("Error! pattern_euclidean.c. There is more than one header in the training set.");
		printf("%s\n",file_tr);
		fclose(fp_tr);
		fclose(fp_tt);
		fclose(fp_temp);
	...
	if(i>6)
	{
		printf("Error! pattern_euclidean.c. There is more than one header in the test set.");
		printf("%s\n",file_tt);
		fclose(fp_tr);
		fclose(fp_tt);
		fclose(fp_temp);
	...
	while ((ch_tt != '*') || (ch_tr != '*'))
	{
		...
		{
			printf("Error! pattern_euclidean.c. The header in the training and test files don't match.");
			fclose(fp_temp);
			fclose(fp_tr);
			fclose(fp_tt);
			*err_code = 1;
	...
	if(!a)
	{
		printf("\nError! pattern_euclidean.c. Memory allocation error.");
		fclose(fp_temp);
		fclose(fp_tr);
		fclose(fp_tt);
		*err_code = 9;
	...
	for(m=0;m<jj;m++)
	{
		...
		for(n=0;n<j;n++)
		{
			...
			if((int)ch_tr == 10)
			{
				printf("Error! pattern_euclidean.c. Every entry in training set should have a class name.");
				fclose(fp_temp);
				fclose(fp_tr);
				fclose(fp_tt);
				*err_code = 10;
			...
			// Calls the euclidean distance measure function
			euclidean = euclidean_distance(a, b, i);
			...
			fprintf(fp_temp,"%d %d %f ", r, c, euclidean);
