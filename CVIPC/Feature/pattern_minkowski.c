...
* =========================================================================
*
*             File Name: pattern_minkowski.c 
*		  Expanded Name: Pattern classification minkowski metric distance
...
extern float minkowski_distance(float *vector1, float *vector2, int r_minkowski, int n, int *err_code);

int pattern_minkowski(char *file_tt, char *file_tr, int r_minkowski, int *err_code)
{	
	float	*a,*b;
	float	minkowski = 0.0;
	...
	{
		printf("Error! pattern_minkowski.c. Can't open , read or create file.");
		printf("%s\n",file_tr);
		*err_code = 7;
		return(1);
	}
	...
	if(i>6)
	{
		printf("Error! pattern_minkowski.c. There is more than one header in the training set.");
		printf("%s\n",file_tr);
		fclose(fp_tr);
		fclose(fp_tt);
		fclose(fp_temp);
	...
	if(i>6)
	{
		printf("Error! pattern_minkowski.c. There is more than one header in the test set.");
		printf("%s\n",file_tt);
		fclose(fp_tr);
		fclose(fp_tt);
		fclose(fp_temp);
	...
	while ((ch_tt != '*') || (ch_tr != '*'))
	{
		...
		{
			printf("Error! pattern_minkowski.c. The header in the training and test files don't match.");
			fclose(fp_temp);
			fclose(fp_tr);
			fclose(fp_tt);
			*err_code = 1;
	...
	if(!a)
	{
		printf("\nError! pattern_minkowski.c. Memory allocation error.");
		fclose(fp_temp);
		fclose(fp_tr);
		fclose(fp_tt);
		*err_code = 9;
	...
	for(m=0;m<jj;m++)
	{
		...
		for(n=0;n<j;n++)
		{
			...
			if((int)ch_tr == 10)
			{
				printf("Error! pattern_minkowski.c. Every entry in training set should have a class name.");
				fclose(fp_temp);
				fclose(fp_tr);
				fclose(fp_tt);
				*err_code = 10;
			...
			// Calls the minkowski distance measure function
			minkowski = minkowski_distance(a, b, r_minkowski,i,err_code);
			if(*err_code !=0)
			{
				fclose(fp_temp);
				fclose(fp_tr);
			...
			fprintf(fp_temp,"%d %d %f ", r, c, minkowski);
