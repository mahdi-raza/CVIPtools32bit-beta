...
static Image *sobel_Image(Image *cvipImage2, Image *directionImage, int size);
static void threshold_image(Image *inputImage,int thres);
static MATRIX *get_mask(MASK_TYPE type, int dimension);

Image *sobel_filter(Image *inputImage, Image *dirImage, int mask_choice, 
	int mask_size, int keep_dc, int threshold)
{
	...
	if(warning)
	fprintf(stderr,"SOBEL_FILTER:kernel size is changed to %d",mask_size);
	*/
	magImage = sobel_Image(inputImage,direction_Image, mask_size);
...
static Image *sobel_Image(Image *cvipImage, Image *directionImage, int size)
{
