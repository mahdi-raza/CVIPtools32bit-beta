/****************************************************************
                   Name: robinson_filter
          Expanded Name: robinson edge detection filter
...
static Image *robinson_edge(Image *inputImage, Image *pdirimage);

Image *robinson_filter(Image *imageP,Image *pdirimage, int mask_choice, 
...
if(mask_choice > 0 && mask_choice < 5)
{
   ...
   magImage = robinson_edge(imageP, direction_Image);
...
/****************************************************************
   ...
   Credit: Kun Luo
*****************************************************************/
static void threshold_image(Image *magnitude_Image,int thres){
   int x, y,row,col;
   float  **real;
   int band;
   row= magnitude_Image->bandP[0]->rows;
   col= magnitude_Image->bandP[0]->cols;
   for(band=0;band<magnitude_Image->bands;band++){
       real=getData_Image(magnitude_Image,band);
       for(y=0;y<row;y++){
	   for(x=0;x<col;x++){
...
/***********************************************************************
* This is new robinson edge function, 2009
*
...
static Image *robinson_edge(Image *inputImage, Image *pdirimage){
    Image  *magImage;
	int r0[]={-1,0,1,-2,0,2,-1,0,1},r1[]={0,1,2,-1,0,1,-2,-1,0},r2[]={1,2,1,0,0,0,-1,-2,-1},r3[]={2,1,0,1,0,-1,0,-1,-2},r4[]={1,0,-1,2,0,-2,1,0,-1},r5[]={0,-1,-2,1,0,-1,2,1,0},r6[]={-1,-2,-1,0,0,0,1,2,1},r7[]={-2,-1,0,-1,0,1,0,1,2};
...
data_type1 = getDataType_Image(inputImage);
 ...
 bands = getNoOfBands_Image(inputImage);   
   ...
   for(no_of_bands=0;no_of_bands<bands;no_of_bands++)
   {
	   ...
	   image_d = getData_Image(pdirimage,no_of_bands);
	   for(y=1; y<rows-1; y++)
	   {
		   for(x=1; x<cols-1; x++)
		   {
