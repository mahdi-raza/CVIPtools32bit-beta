    ...
    ======================================================================
          ...
          Expanded Name: Zonal coding
			 ...
			 compress_ratio -- compression ratio.
                ...
                Returns: For zon_compress(), if succeeds, return 1; if errors
			 happen, return 0.

			 For zon_decompress(), returns pointer to the 
			 decompressed image.
...
static void dealing_remap(long rows, long cols, float * rp, byte * sp);
...
static Image * image_norm(Image * inputImage, unsigned int newrows,
...
static Image * imag_restore(float * newdata, int * P, int rows, int cols, 
			    long block_size, char choice);
static void conjugate(Image * inputImage, long block_size);
static Image * inv_xform_calling(Image * inputImage,
			int block_size, int choice);
...
static remapSTRUCT * sampling(Image * inputImage, int * P, int block_size)
{
   ...
   rer = (float *) calloc(vbn*hbn*computing(P, block_size), sizeof(float));
   if (complex) rei = (float *) calloc(vbn*hbn*computing(P, block_size), sizeof(float));
...
/* in one block */
      { position = 0;
		...
		{
		  rer[(i*hbn+j)*len +position] = real[x][y]; 
        	  if (complex) rei[(i*hbn+j)*len +position] = imag[x][y];
		  position++;
		}
      }
	   ...
	   int bands, COLOR_FORMAT color, unsigned int rows, unsigned int cols)
{
  ...
  if (fwrite("ZON", 1, 3, fp) != 3)
     { fprintf(stderr,"write ZON error\n"); return 0;}
   ...
   for (i=0; i < rows; i++)
	...
	for (j=0; j < cols; j++, rp++)
        ...
        *((float *)(sp + sizeof(float))) = factor;
        sp += 2 * sizeof(float);
        rp -= cols;
        for (j=0; j<cols; j++, sp++, rp++)
          *sp = factor * (*rp - min) + 0.5;
     }
}
...
static storeSTRUCT * newremap(remapSTRUCT * remapp)
{
   ...
   storep = (storeSTRUCT *)malloc(sizeof(storeSTRUCT));
   storep->rows = remapp->rows;
   storep->cols = remapp->cols + 2*sizeof(float);
   ...
   dealing_remap(remapp->rows, remapp->cols, remapp->rer, storep->storer);
   if (remapp->rei != NULL)
     dealing_remap(remapp->rows, remapp->cols, remapp->rei, storep->storei);
...
int zon_compress(Image * inputImage, char *filename, int block_size,
...
static float * inv_remap(unsigned char * data, long * temp)
{
  ...
  for (i=0; i<temp[0]; i++)
   { minP    = *((float *)(data + i*temp[1]));
     ...
     for (j=0; j<cols; j++)
       { 
         *newdata = (float)(minP + ((float)data[i*temp[1] + 2*sizeof(float) + j] - 0.5)/factor);
         newdata++;
        }
   }
			  ...
			  unsigned int rows, unsigned int cols)
{
  ...
  Image        * cvipImage,* imagImage;
   ...
   cvipImage = imag_restore(rnewdata, P, rows, cols, block_size, choice);
   if (choice == 1) 
     { imagImage = imag_restore(inewdata, P, rows, cols, block_size, choice);  
...
Image * zon_decompress(char * inputfile)
{ FILE           * fp;
  Image          * finalImage;
  ...
  finalImage = zon_decompression(fp);
  fclose(fp);

  return finalImage;
}

Image * zon_decompression(FILE * fp)
{
  struct stat      st; 
  Image          * finalImage;
  char             choice, ***iptr, a[4];
  long             block_size;
  IMAGE_FORMAT     format;
  unsigned char    bands;
  COLOR_FORMAT	   color;
