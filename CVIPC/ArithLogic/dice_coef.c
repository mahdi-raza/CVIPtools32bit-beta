...
*
*             File Name: Dice.c
*           Description: Finds the DICE Coefficient of two images.		
*         Related Files: 
...
double* dice_coef(Image* pIdealImage, Image* pTestImage)
{
	Image* andImage;
	...
	float idealArea, testArea, intersectArea, andArea, tArea, iArea;
	...
	if (binary == 1)
	{
		for (nBand = 0; nBand < nBands; nBand++)
		{
			...
			if (idealArea != 0)
				fRatio[nBand] = (2 * intersectArea) / (idealArea + testArea);
		}
	}
	else
	{
		...
		for (nBand = 0; nBand < nBands; nBand++)
		{
			...
			for (nRow = 0; nRow < nRows; nRow++)
			{
				for (nCol = 0; nCol < nCols; nCol++)
				{
					...
					tArea += pTestData[nRow][nCol];
					iArea += pIdealData[nRow][nCol];
				}
			}
			fRatio[nBand] = (2 * andArea) / (tArea + iArea);
		}
