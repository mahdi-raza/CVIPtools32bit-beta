...
*
*             File Name: Dice.c
*           Description: Finds the Overlap Coefficient of two images.
*         Related Files:
...
double* overlap_coef(Image* pIdealImage, Image* pTestImage)
{
	Image* andImage;
	byte** pIdealData, ** pTestData, ** andData;
	double* fRatio;
	int nBand, nBands, nBandsIdeal, nBandsTest;
	int nRow, nRows, nCol, nCols;
	float idealArea, testArea, intersectArea, andArea, minArea;
	...
	if (binary == 1)
	{
		for (nBand = 0; nBand < nBands; nBand++)
		{
			...
			if (idealArea <= testArea)
				minArea = idealArea;
			else
				minArea = testArea;

			if (idealArea != 0)
				fRatio[nBand] = intersectArea / minArea;
		}
	}
	else
	{
		...
		for (nBand = 0; nBand < nBands; nBand++)
		{
			...
			for (nRow = 0; nRow < nRows; nRow++)
			{
				for (nCol = 0; nCol < nCols; nCol++)
				{
					...
					testArea += pTestData[nRow][nCol];
					idealArea += pIdealData[nRow][nCol];
				}
			}
			if (idealArea <= testArea)
				minArea = idealArea;
